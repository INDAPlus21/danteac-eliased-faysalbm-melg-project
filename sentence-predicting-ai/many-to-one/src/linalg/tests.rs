...
mod tests {
    ...
    #[test]
    fn test_vector_argmax() {
        assert_eq!(Vector::from_vec(vec![3.0, 2.0, 7.0]).argmax(), 2.0 as usize);

        assert_ne!(Vector::from_vec(vec![5.0, 1.0, 3.0]).argmax(), 1.0 as usize);
    }
    //endregion
    ...
    */// Missing
    ...
    fn test_vector_add_assign() {
        ...
        let mut vector2: Vector = Vector::from_vec(vec![1.0, 3.0, 5.0]);
        vector1 += vector2;
        assert_eq!(
            vector1,
            Vector::from_vec(vec![2.0, 6.0, 10.0])
        ...
        assert_ne!(
            vector1,
            Vector::from_vec(vec![2.0, 6.0, 12.0])
        );
    }
    //endregion
    ...
    fn test_matrix_add_assign() {
        let mut matrix1: Matrix = Matrix::from_vecs(vec![
            vec![3.0, -5.0],
            vec![-7.0, 6.0],
        ]);
        let mut matrix2: Matrix = Matrix::from_vecs(vec![
            vec![4.0, 8.0],
            vec![-2.0, 9.0],
        ]);
        matrix1 += matrix2;
    ...
    #[test]
    fn test_matrix_mul() {
        let a: Matrix = Matrix::from_vecs(vec![
            vec![1.0, 5.0, 4.0],
            vec![3.0, 15.0, -7.0]
        ]);
        let b: Matrix = Matrix::from_vecs(vec![
            vec![2.0, 3.0],
            vec![4.0, -1.0],
            vec![-5.0, 8.0]
        ]);
        let output: Matrix = Matrix::from_vecs(vec![
            vec![2.0, 30.0],
            vec![101.0, -62.0]
        ]);
        assert_eq!(a * b, output);
