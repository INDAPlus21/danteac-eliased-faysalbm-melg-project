...
impl Matrix {
    ...
    pub fn from_vecs(vecs: Vec<Vec<f32>>) -> Matrix {
        let mut vec_of_vecs: Vec<Vector> = vec![];
        for vec in vecs {
    ...
    pub fn with_random_normal(height: usize, width: usize, mean: f32, std_dev: f32) -> Matrix {
        let mut vec_of_vectors: Vec<Vector> = vec![];
        for _ in 0..height {
    ...
    pub fn get_content_as_string(&self) -> String{
        let mut string: String = Matrix::get_width(&self).to_string();
        string.push_str(" ");
        string.push_str(&Matrix::get_height(&self).to_string());
        string.push_str(" ");
