...
impl Vector {
    ...
    pub fn with_random_normal(length: usize, mean: f32, standard_deviation: f32) -> Vector {
        let normal_distrubution: Normal<f32> =
            rand_distr::Normal::new(mean, standard_deviation).unwrap();
        let mut vec: Vec<f32> = vec![0.0; length];
        for i in 0..vec.len() {
            vec[i] = normal_distrubution.sample(&mut rand::thread_rng());
        }
        Vector { vector: vec }
    }
    //endregion
