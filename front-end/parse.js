...
fs.readFile(midi_file, 'base64', function (err, raw_data) {
    ...
    let midiArray = midiParser.parse(raw_data);
    ...
    function getNotesAndTimes() {
        ...
        for (let i = 0; i < track.length; i++) {
            let data = track[i].data
            let type = track[i].type
            let deltaTime = track[i].deltaTime
            // var deltaTime_2 = track_2[i].deltaTime
            // var data_2 = track_2[i].data
            if (type == 9) {
                let note = notes[data[0]]
    ...
    for (let i = 0; i < midiArray.track.length; i++) { 
        var track = midiArray.track[i].event // [2].event
        let notes_and_times = getNotesAndTimes()
    ...
    let combined = combineTracks(tracks[0], tracks[1])
